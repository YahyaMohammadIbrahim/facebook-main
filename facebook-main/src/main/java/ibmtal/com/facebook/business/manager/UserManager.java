...
public class UserManager implements UserService {
	...
	if(this.userDao.getByMail(user.getMail()).isEmpty()==false) {
	  ...
	  if(user.getName().isBlank()) {
		  result.setSuccess(false);
		mesaj.add(new Message("name",user.getName() + "Adınızı boş bırakamazsınız!"));
		
	}
	  if(user.getSurname().isBlank()) {
		  result.setSuccess(false);
		  mesaj.add(new Message("surname",
				    "Soyadınızı boş bırakamazsınız!"));
	  }
	  if(user.getUsername().length()<3) {
		  result.setSuccess(false);
		  mesaj.add(new Message("username",
				    "Kullanıcı adınız 3 karakterden fazla olmalıdır!"));
	  }
	  if(user.getName().length()<2) {
		  result.setSuccess(false);
		  mesaj.add(new Message("name", 
				    " Adınız 2 karakterden fazla olmalıdır!"));
	  }
	  if(result.isSuccess()) {
		  userDao.save(user);
	  }
